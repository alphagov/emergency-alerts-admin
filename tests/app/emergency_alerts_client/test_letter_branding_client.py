...
from app.emergency_alerts_client.letter_branding_client import LetterBrandingClient


def test_get_letter_branding(mocker, fake_uuid):
    mock_get = mocker.patch(
        "app.emergency_alerts_client.letter_branding_client.LetterBrandingClient.get", return_value={"foo": "bar"}
    )
...
def test_get_all_letter_branding(mocker):
    mock_get = mocker.patch(
        "app.emergency_alerts_client.letter_branding_client.LetterBrandingClient.get", return_value=[1, 2, 3]
    )
    mock_redis_get = mocker.patch("app.extensions.RedisClient.get", return_value=None)
    mock_redis_set = mocker.patch("app.extensions.RedisClient.set")
...
def test_create_letter_branding(mocker):
    ...
    mock_post = mocker.patch("app.emergency_alerts_client.letter_branding_client.LetterBrandingClient.post")
