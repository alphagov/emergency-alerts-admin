...
from app.emergency_alerts_client.job_api_client import JobApiClient
...
def test_client_creates_job_data_correctly(mocker, fake_uuid):
    ...
    mock_post = mocker.patch(
        "app.emergency_alerts_client.job_api_client.JobApiClient.post",
...
def test_client_schedules_job(mocker, fake_uuid):
    ...
    mock_post = mocker.patch("app.emergency_alerts_client.job_api_client.JobApiClient.post")
...
def test_client_gets_job_by_service_and_job(mocker):
    ...
    client = JobApiClient()
    mock_get = mocker.patch("app.emergency_alerts_client.job_api_client.JobApiClient.get")
...
def test_client_parses_job_stats(mocker):
    ...
    expected_data = {
        "data": {
            ...
            "original_file_name": "test-emergency-alerts-email.csv",
            "created_by": {
    ...
    mock_get = mocker.patch("app.emergency_alerts_client.job_api_client.JobApiClient.get", return_value=expected_data)
...
def test_has_jobs_sets_cache(
    ...
    mock_get = mocker.patch(
        "app.emergency_alerts_client.job_api_client.JobApiClient.get", return_value={"data": job_data}
    )
    mock_redis_set = mocker.patch("app.extensions.RedisClient.set")
