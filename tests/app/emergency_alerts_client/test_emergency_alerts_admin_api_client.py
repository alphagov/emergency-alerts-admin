...
def test_active_service_can_be_modified(emergency_alerts_admin, method, user, service):
    api_client = EmergencyAlertsAdminAPIClient()
...
@pytest.mark.parametrize("method", ["put", "post", "delete"])
def test_inactive_service_cannot_be_modified_by_normal_user(emergency_alerts_admin, api_user_active, method):
    api_client = EmergencyAlertsAdminAPIClient()
...
@pytest.mark.parametrize("method", ["put", "post", "delete"])
def test_inactive_service_can_be_modified_by_platform_admin(emergency_alerts_admin, platform_admin_user, method):
    api_client = EmergencyAlertsAdminAPIClient()
...
def test_generate_headers_sets_standard_headers(emergency_alerts_admin):
    api_client = EmergencyAlertsAdminAPIClient()
    with set_config(emergency_alerts_admin, "ROUTE_SECRET_KEY_1", "proxy-secret"):
        api_client.init_app(emergency_alerts_admin)

    # with patch('app.emergency_alerts_client.has_request_context', return_value=False):
    ...
    assert headers["User-agent"].startswith("EMERGENCY-ALERTS-API-PYTHON-CLIENT")
    assert headers["X-Custom-Forwarder"] == "proxy-secret"


def test_generate_headers_sets_request_id_if_in_request_context(emergency_alerts_admin):
    api_client = EmergencyAlertsAdminAPIClient()
    api_client.init_app(emergency_alerts_admin)

    with emergency_alerts_admin.test_request_context() as request_context:
        headers = api_client.generate_headers("api_token")
