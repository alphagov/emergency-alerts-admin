...
def mock_emergency_alerts_client_check_inactive_service(mocker):
    mocker.patch("app.emergency_alerts_client.EmergencyAlertsAdminAPIClient.check_inactive_service")
...
def test_client_gets_all_users_for_service(
    ...
    mock_get = mocker.patch(
        "app.emergency_alerts_client.user_api_client.UserApiClient.get",
        return_value={
            "data": [
                {"id": fake_uuid},
            ]
...
def test_client_uses_correct_find_by_email(mocker, api_user_active):
    ...
    mock_post = mocker.patch("app.emergency_alerts_client.user_api_client.UserApiClient.post")
...
def test_client_only_updates_allowed_attributes(mocker):
    mocker.patch("app.emergency_alerts_client.current_user", id="1")
...
def test_client_updates_password_separately(mocker, api_user_active):
    ...
    user_api_client.max_failed_login_count = 1  # doesn't matter for this test
    mock_update_password = mocker.patch("app.emergency_alerts_client.user_api_client.UserApiClient.post")
...
def test_client_converts_admin_permissions_to_db_permissions_on_edit(emergency_alerts_admin, mocker):
    mock_post = mocker.patch("app.emergency_alerts_client.user_api_client.UserApiClient.post")
...
def test_client_converts_admin_permissions_to_db_permissions_on_add_to_service(emergency_alerts_admin, mocker):
    mock_post = mocker.patch(
        "app.emergency_alerts_client.user_api_client.UserApiClient.post", return_value={"data": {}}
    )
...
def test_returns_value_from_cache(
    ...
    mock_api_get = mocker.patch(
        "app.emergency_alerts_client.EmergencyAlertsAdminAPIClient.get",
...
def test_deletes_user_cache(emergency_alerts_admin, mock_get_user, mocker, client, method, extra_args, extra_kwargs):
    mocker.patch("app.emergency_alerts_client.current_user", id="1")
...
def test_get_webauthn_credentials_for_user(mocker, webauthn_credential, fake_uuid):
    mock_get = mocker.patch(
        "app.emergency_alerts_client.user_api_client.UserApiClient.get", return_value={"data": [webauthn_credential]}
    )
...
def test_complete_webauthn_login_attempt_returns_false_and_message_on_403(fake_uuid, mocker):
    mock_post = mocker.patch(
        "app.emergency_alerts_client.user_api_client.UserApiClient.post",
