...
@pytest.mark.parametrize(
    ...
    [
        ("me@example.com", "Enter a public sector email address"),
...
def test_choosing_weak_password_returns_400(client_request, mock_verify_password, mocker):
    mocker.patch(
        ...
        side_effect=HTTPError(
            Response(status=400),
            ["Your password is not strong enough, try adding more words"],
        ),
    )
    ...
    assert normalize_spaces(page.select_one(".govuk-error-message").text) == (
        "Error: Your password is not strong enough, try adding more words"
    )
