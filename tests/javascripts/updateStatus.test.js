...
describe('Update content', () => {
  ...
  beforeEach(() => {
    ...
    document.body.innerHTML = `
      ...
      <div data-emergency-alerts-module="update-status" data-updates-url="${updatesURL}" data-target="template_content">
        Initial content
      </div>
    `;
  ...
  test("It should add attributes to the elements", () => {
    ...
    expect(
      document.querySelectorAll('[data-emergency-alerts-module=update-status]')[0].id
    ).toEqual(
      "update-status"
    );
  ...
  test("It should replace the content of the div with the returned HTML", () => {
    ...
    expect(
      document.querySelectorAll('[data-emergency-alerts-module=update-status]')[0].textContent.trim()
    ).toEqual(
      "Initial content"
    );
