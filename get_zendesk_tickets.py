...
EMERGENCY_ALERTS_ZENDESK_EMAIL = "zd-api-emergency-alerts@digital.cabinet-office.gov.uk"
ZENDESK_API_KEY = os.environ.get("ZENDESK_API_KEY")


def get_tickets():
    ZENDESK_TICKET_URL = "https://govuk.zendesk.com/api/v2/search.json?query={}"
    query_params = "type:ticket group:{}".format(EMERGENCY_ALERTS_GROUP_ID)
    ...
    with open("zendesk_ticket_data.csv", "w") as csvfile:
        ...
        while next_page:
            ...
            response = requests.get(
                ...
                auth=("{}/token".format(EMERGENCY_ALERTS_ZENDESK_EMAIL), ZENDESK_API_KEY),
            )
            data = response.json()
            print(data)
...
def get_tickets_with_description():
    ZENDESK_TICKET_URL = "https://govuk.zendesk.com/api/v2/search.json?query={}"
    query_params = "type:ticket group:{}, created>2019-07-01".format(EMERGENCY_ALERTS_GROUP_ID)
    query_params = urllib.parse.quote(query_params)
